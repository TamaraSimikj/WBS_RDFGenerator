...
public class GeneratorService {
    public String generateDataForShapeInput(String shape, Integer numberOfExamples) {

        String outputPath = "C:\\Users\\Tamara\\IdeaProjects\\RDFGenerator\\src\\main\\resources\\generatedData\\generated_data.ttl";
        Model shapeModel = ModelFactory.createDefaultModel();
        shapeModel.read(new StringReader(shape), null, "TURTLE");

        try (OutputStream out = new FileOutputStream(outputPath,false)) {
            for (int i = 0; i < numberOfExamples; i++) {
                Model generatedData = generateDataModel(shapeModel);
                // Write the generated data to the output file
                RDFDataMgr.write(out, generatedData, Lang.TURTLE);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return "Generating file failed"; // Return null to indicate an error
        }
    ...
    public String generateDataFileForShapePath(String fileName, Integer numberOfExamples) {
//        String outputPath = new File("generatedData", "generated_data.ttl").getAbsolutePath();
      String outputPath = "C:\\Users\\Tamara\\IdeaProjects\\RDFGenerator\\src\\main\\resources\\generatedData\\generated_data.ttl";
        Model shapeModel = ModelFactory.createDefaultModel();
    ...
    private static Model generateDataModel(Model shapeModel) {
        ...
        // Iterate over the shape model and create data based on the shape
        StmtIterator shapeIterator = shapeModel.listStatements(null, RDF.type, SHACLM.Shape);
        if(!shapeIterator.hasNext()){
            shapeIterator = shapeModel.listStatements(null, RDF.type, SHACLM.NodeShape);
            if(!shapeIterator.hasNext()){
                shapeIterator = shapeModel.listStatements(null, RDF.type, SHACLM.PropertyShape);

            }
        }
        while (shapeIterator.hasNext()) {
            Statement shapeStatement = shapeIterator.nextStatement();
            Resource shapeResource = shapeStatement.getSubject();
